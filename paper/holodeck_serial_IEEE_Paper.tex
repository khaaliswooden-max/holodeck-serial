...
empirical CPython reference measurement (\(\sim\)371 ticks/sec over a 60\,s
single-core run; host-dependent, $\sim$310--370 across runs), which clears the
threshold by \(\sim6\times\) despite a large
...
This figure is analytical (idealized 1-op-per-cycle model). A CPython reference
implementation, bearing the full interpreter tax, sustains $\sim$371 ticks/sec
($\sim$6$\times$ headroom) on a single core---so the qualitative serial-sufficiency
claim holds empirically, while the $\sim$2{,}700$\times$ margin remains a property
of the analytical model. Reproduction on declared baseline hardware constitutes
the primary next research phase.
